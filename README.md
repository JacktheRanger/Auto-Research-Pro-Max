...
- Backend: [FastAPI](https://fastapi.tiangolo.com/), [SQLite](https://www.sqlite.org/), [OpenAI Python SDK](https://github.com/openai/openai-python), [pypdf](https://pypdf.readthedocs.io/), [PyMuPDF](https://pymupdf.readthedocs.io/)
...
- The Docker sandbox currently runs a controlled synthetic experiment stub rather than repository-aware real workloads.
- Final export, bibliography generation, citation verification, claim-evidence checks, and richer review loops are still in progress.
...
- Replace the synthetic sandbox stub with repository-aware experiment execution and stronger recovery controls.
- Add branching, per-stage retry policies, cost tracking, and project templates.
- Add real export pipelines for `Markdown`, `LaTeX`, and compiled `PDF`.
...
当前产品采用固定的 15 阶段流程加一个计划审批门，并在实验设计、沙箱评审和论文修订处设置了显式审批门。按项目自定义阶段列表、分支对比和更丰富的重试策略仍然是后续工作。
...
- 后端：[FastAPI](https://fastapi.tiangolo.com/)、[SQLite](https://www.sqlite.org/)、[OpenAI Python SDK](https://github.com/openai/openai-python)、[pypdf](https://pypdf.readthedocs.io/)、[PyMuPDF](https://pymupdf.readthedocs.io/)
...
- 当前 Docker 沙箱执行的仍然是受控的 synthetic experiment stub，而不是仓库感知的真实实验工作负载。
- 最终导出、参考文献生成、引文校验、claim-evidence 检查和更丰富的审阅闭环仍在开发中。
- Windows 一键启动目前还不可用。

## 路线图

- 增加针对扫描版或解析不完整论文的 OCR 回退与手工 metadata 修正流程。
- 把当前 synthetic sandbox stub 替换成仓库感知的真实实验执行与更强的恢复控制。
- 增加分支对比、阶段重试策略、成本跟踪和项目模板。
- 增加真实的 `Markdown`、`LaTeX` 和编译后 `PDF` 导出流水线。
