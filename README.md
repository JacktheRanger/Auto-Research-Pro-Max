...
- `macOS` or `Windows 10/11` for the one-click launcher flow
- `Python 3.11+` available on `PATH` (Windows recommended: install via `py -3` launcher)
- `Node.js 18+` and `npm`
...
On Windows, double-click [`start.cmd`](start.cmd) (`start-lan.cmd` for LAN sharing, `stop.cmd` to shut the server down).
...
To stop the background server later, double-click [`stop.command`](stop.command) on macOS or [`stop.cmd`](stop.cmd) on Windows.
...
- 一键启动流程支持 `macOS` 与 `Windows 10/11`
- `Python 3.11+`（Windows 推荐安装 `py -3` 启动器）
- `Node.js 18+` 和 `npm`
...
在 Windows 上，直接双击 [`start.cmd`](start.cmd)（局域网模式使用 `start-lan.cmd`，停止服务使用 `stop.cmd`）。
...
如果之后需要停止后台服务，在 macOS 上双击 [`stop.command`](stop.command)，在 Windows 上双击 [`stop.cmd`](stop.cmd)。
